...
const ETH_NETWORK_RPC_PROVIDER =
  process.env.ETH_NETWORK_RPC_PROVIDER || `https://${ETH_NETWORK}.optimism.io`
const ETH_REPLICA_RPC_PROVIDER =
  process.env.ETH_REPLICA_RPC_PROVIDER || 'http://localhost:8545'
...
export const runSyncCheck = async (metrics?: ReplicaMetrics) => {
  const sequencerProvider = injectL2Context(
    new providers.JsonRpcProvider(ETH_NETWORK_RPC_PROVIDER)
  )
  const replicaProvider = injectL2Context(
    new providers.JsonRpcBatchProvider(ETH_REPLICA_RPC_PROVIDER)
  )
  ...
  while (true) {
    ...
    console.log(
      `Current height difference from sequencer: ${
        sequencerLatest.number - replicaLatest.number
      }`
    )
    if (metrics) {
